...
constexpr unsigned ERROR_STATUS = -1;
constexpr unsigned OK_STATUS    = 0;
constexpr std::size_t numb_rows = 200;
constexpr std::size_t numb_cols = 300;
...
void FindAverageValues( 
    const eprocess_type proc_type, 
    const double** matrix, 
    const size_t numb_rows, 
    const size_t numb_cols, 
    double* average_vals ) 
{
	switch ( proc_type ) {
...
void PrintAverageVals( const eprocess_type proc_type, const double* average_vals, const size_t dimension ) {
	switch ( proc_type ) {
		case eprocess_type::by_rows: {
